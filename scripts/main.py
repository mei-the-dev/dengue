...
def main():
    ...
    data_file = project_dir / "data" / "Dengue_Brasil_2010-2016_RJ.xlsx"
    ...
    # =========================================================================
    print_section("Validação inicial dos dados unificados")
    # Carregar dados já padronizados
    df_check = load_and_merge_dengue_data(data_file)
    print(f"  Registros: {len(df_check)}")
    print(f"  Colunas: {df_check.columns.tolist()}")
    print(f"  Valores ausentes por coluna:\n{df_check.isnull().sum()}")
    if (df_check['casos'] < 0).any():
